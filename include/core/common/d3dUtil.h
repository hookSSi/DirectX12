...
class d3dUtil
{
    public:
        ...
        LoadBinary(const std::wstring &filename);
        ...
        static Microsoft::WRL::ComPtr<ID3DBlob>
        CompileShader(const std::wstring &filename,
...
struct Light
{
        ...
        DirectX::XMFLOAT3 Direction = {0.0f, -1.0f,
                                       0.0f}; // directional/spot light only
        float FalloffEnd = 10.0f;             // point/spot light only
        DirectX::XMFLOAT3 Position = {0.0f, 0.0f,
                                      0.0f}; // point/spot light only
...
struct MaterialConstants
{
        ...
        DirectX::XMFLOAT4X4 MatTransform = Common::MathHelper::Identity4x4();
};
...
struct Material
{
        ...
        // Dirty flag indicating the material has changed and we need to update
        // the constant buffer. Because we have a material constant buffer for
        // each FrameResource, we have to apply the update to each
        // FrameResource.  Thus, when we modify a material we should set
        // NumFramesDirty = gNumFrameResources so that each frame resource gets
        // the update.
